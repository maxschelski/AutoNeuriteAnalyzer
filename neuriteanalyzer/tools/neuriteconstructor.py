...
class neuriteConstructor():
    ...
    def __init__(self,allNeuritePoints,labeledGroupImage,cX,cY,
                 ...
                 minBranchSize,testNb,optimalThreshold,maxOverlapOfNeurites,
                 img,toleranceForSimilarity,dilationForSmoothing,
                 gaussianForSmoothing):
    ...
    def constructNeurites(self):
        ...
        for neuritePoints in self.allNeuritePoints:
            ...
            self.constructedNeuriteImage = generalTools.getImageFromPoints(neuritePoints,
                                                                           self.img.shape)

            self.constructedNeuriteImage = generalTools.smoothenImage(self.constructedNeuriteImage,
            ...
            startPoint = generalTools.getClosestPoint(self.allCoords,
                                                      [[neuritePoints[0][0],
                                                        neuritePoints[0][1]]])
        ...
        (self.neuriteOrigin,
         self.newNeuritesDf) = self.setOriginIfNoneWasFound(self.neuriteOrigin,
                                                            self.newNeuritesDf)

        (self.newNeuritesDf,
         allOrigins) = self.setRemainingOriginsOfNeurites(self.newNeuritesDf,
                                                            self.neuriteOrigin)

        #set branch number similar to previous timeframes
        (self.newNeuritesDf,
         allOrigins) = self.checkIfBranchesWithSameOriginOverlap(self.newNeuritesDf)

        #set remaining branches which were not found
        #in previous timeframes
        self.newNeuritesDf = self.setRemainingBranchesOfNeurites(allOrigins,
                                                                 self.newNeuritesDf,
                                                                 neuriteBranch)

        #check if branches are doubled for same origin, only keep longest branch
        self.newNeuritesDf = self.removeDoubledBranches(allOrigins,self.newNeuritesDf,
                                                        self.allNeurites)
        ...
        (self.allNeurites,
         self.newNeuritesDf) = self.checkForOverlapBetweenDifferentOrigins(self.overlappingOrigins,
                                                                           self.allNeurites,
                                                                           self.newNeuritesDf)
    ...
    def getNextOrigin(self,newNeuritesDf,origin):
        ...
        newNeuritesDfForNextOrigin = newNeuritesDf.loc[newNeuritesDf['origin'] !=
                                                       origin]
        nextOrigin = max(nextOriginAllNeurites,
                         np.max(newNeuritesDfForNextOrigin['origin'])+1)
        return nextOrigin
    ...
    def groupBranchesByOverlap(self,oneOrigin,min_branch_overlap_factor=-1):
        ...
        for oneIndex in oneOrigin.index.values:
            ...
            if (not indexAlreadySorted) & (no_corresponding_neurite):
                ...
                firstImg = ndimage.binary_dilation(firstImg,
                                                   disk(self.dilationForOverlap /
                                                        self.overlapMultiplicator),
                                                   iterations=self.overlapMultiplicator)
                for otherIndex in oneOrigin.index.values:
                    indexAlreadySorted = self.checkIfIndexIsAlreadySorted(otherIndex,
                                                                          allOverlappingGroups)
                    ...
                    if min_branch_overlap_factor == -1:
                        ...
                        no_corresponding_neurite = True
                    if ((not indexAlreadySorted) &
                            (otherIndex != oneIndex) &
                            (no_corresponding_neurite)):
                        secondImg = generalTools.getImageFromCoords(secondCoords,
                                                                    self.img.shape)
                        secondImg = ndimage.binary_dilation(secondImg,
                                                            disk(self.dilationForOverlap /
                                                                 self.overlapMultiplicator),
                                                            iterations=self.overlapMultiplicator)
                        (diffOfImgs,
                         overlapOfImgs) = generalTools.overlapImgs(firstImg,
                                                                    secondImg,
    ...
    def getOriginFromPreviousTimeframes(self,overlappingOrigins,neuriteOrigin):                        
        ...
        while continueSearch:
            ...
            if(self.identity[4]-frameTimer) <= 0:
                ...
                frameTimerNeurites = self.allNeurites[(((self.allNeurites.date == self.identity[0]) &
                                                        (self.allNeurites.experiment == self.identity[1])) &
                                                       ((self.allNeurites.neuron == self.identity[2]) &
                                                        (self.allNeurites.channel == self.identity[3]))) &
                                                      (self.allNeurites.time == (self.identity[4]-frameTimer))].reset_index()
                
                (possibleOrigins,
                 overlappingOrigins) = self.getPossibleOriginFromList(self.sortedArray,
                                                                        frameTimerNeurites,
                                                                        possibleOrigins,
                                                                        overlappingOrigins,
                                                                        len(self.newNeuritesDf),
                                                                        self.dilationForOverlap,
                                                                        self.overlapMultiplicator,
                                                                        frameTimer)
                frameTimer += 1
        ...
        if(len(possibleOrigins) > 0):
            ...
            bestOrigin = neuriteConstructor.getBestNeuriteBasedOnSimilarityAndFrequency(possibleOrigins,
                                                                                        self.allNeurites)
#            bestOrigin = self.getRowWithMaximumVal(possibleOrigins,'pxSimilar')
        ...
        if 'bestOrigin' not in locals():
            ...
            bestOrigin.loc[0] = [np.nan,np.nan,np.nan,np.nan,np.nan,[],[],[],[],
                                 self.maxChangeOfNeurite*2,
                                 self.maxChangeOfNeurite*2,0,0]
            bestOrigin = bestOrigin.loc[0]
                        
        return (neuriteOrigin, bestOrigin, neuriteBranch,start_branch,
                overlappingOrigins)
    ...
    def setOriginIfNoneWasFound(self,neuriteOrigin,newNeuritesDf):
        ...
        if np.isnan(neuriteOrigin):
            if len(self.allNeurites) != 0:
                allFrameNeurites = self.allNeurites[((self.allNeurites.date == self.identity[0]) &
                                                     (self.allNeurites.experiment == self.identity[1])) &
                                                    ((self.allNeurites.neuron == self.identity[2]) &
                                                     (self.allNeurites.channel == self.identity[3]))].reset_index()
    ...
    def setRemainingOriginsOfNeurites(self,newNeuritesDf,neuriteOrigin):
        ...
        for rowIndex in newNeuritesDf.index.values:
            ...
            if not np.isnan(thisNeurite['origin']):
                continue
            ...
            for thisOrigin in allOrigins:
                ...
                (possibleOrigins,
                 overlappingOrigins_temp) = self.getPossibleOriginFromList([thisNeurite['x'],
                                                                          thisNeurite['y']],
                                                                         thisOriginNewNeurites,
                                                                         possibleOrigins,
                                                                         overlappingOrigins_temp,
                                                                         rowIndex,
                                                                         self.dilationForOverlap,
                                                                         self.overlapMultiplicator,
                                                                         0,self.constructedNeuriteImage.shape[0],
                                                                         -1,0)

            if len(possibleOrigins) > 0:
                bestOrigin = neuriteConstructor.getRowWithMaximumVal(possibleOrigins,
                                                                     'pxSimilar')['origin']
            else:
    ...
    def getNextBranchNb(self,thisOrigin,newNeuritesDf,allNeurites,identity):
        thisOriginNeurites = allNeurites[(((allNeurites.date == identity[0]) &
                                           (allNeurites.experiment == identity[1])) &
                                          ((allNeurites.neuron == identity[2]) &
                                           (allNeurites.channel == identity[3]))) &
                                         (allNeurites.origin == thisOrigin)].reset_index()
        oldNeuriteBranch = thisOriginNeurites.branch.max() + 1
        thisOriginNewNeurites = newNeuritesDf[newNeuritesDf.origin ==
                                              thisOrigin].reset_index()
    ...
    def setRemainingBranchesOfNeurites(self,allOrigins,newNeuritesDf,
                                       neuriteBranch):
        #if neurites of different origins were determined,
        # treat neurites for each origin seperately
        for thisOrigin in allOrigins:
            #set branch for each neurite where it was not set even if origin
            # was found (for new branchs that emerged)
            #set starting neuriteBranch as first not used branch number      
            if len(self.allNeurites) != 0:
                neuriteBranch = self.getNextBranchNb(thisOrigin,
                                                     self.newNeuritesDf,
                                                     self.allNeurites,
                                                     self.identity)
            ...
            for newNeuritesRowNb in thisOriginNewNeurites.index.values:
                ...
                if np.isnan(thisRow['branch']):
                    ...
                    newNeuritesDf.loc[newNeuritesRowNb,
                                      'start_branch'] = neuriteBranch
                    neuriteBranch += 1
        return newNeuritesDf
    
    @staticmethod
    def getBestNeuriteBasedOnSimilarityAndFrequency(allNeuritesToCompare,
                                                    allNeurites,
                                                    checkFrequency=True):
        maxPxSimilar = max(allNeuritesToCompare.loc[:,'pxSimilar'])
        #find all rows within tolerance of max nb of similar px
        #if frequency is checked, the most important criteria is the frequency,
        # only if similar px deviate a lot (>20%)
        # choose the not most frequently occuring neurite!
        if checkFrequency:
            max_deviation = 0.2
        else:
            max_deviation = 0.1
        mostSimilarNeurites = allNeuritesToCompare.loc[allNeuritesToCompare['pxSimilar'] >=
                                                       (maxPxSimilar * (1 - max_deviation))]
        ...
        if (len(allNeurites) > 0) & checkFrequency:
            ...
            for rowNb in mostSimilarNeurites.index.values:
                ...
                frequency = len(allNeurites.loc[(allNeurites['origin'] == origin) &
                                                (allNeurites['branch'] == branch)])
            ...
            if np.isnan(mostCommonNeurite[0]):
                ...
                bestBranch = mostSimilarNeurites.loc[(mostSimilarNeurites['origin'] == mostCommonNeurite[0]) &
                                                     (mostSimilarNeurites['branch'] == mostCommonNeurite[1])].reset_index().iloc[0]
        if (not checkFrequency) | frequencyCheckFailed:
            ...
            mostSimilarNeurites = mostSimilarNeurites.loc[mostSimilarNeurites['pxdifference'] <=
                                                          (minPxDifference + 5)]
            #if they are similar still, choose neurite with highest average intensity
            bestBranch = neuriteConstructor.getRowWithMaximumVal(mostSimilarNeurites,
                                                                 'avIntOfPoints')
        return bestBranch
    ...
    def remove_overlapping_branches(self,all_origins,newNeuritesDf):
        ...
        if len(newNeuritesDf) != 0:
            for this_origin in all_origins:     
                ...
                grouped_branches = self.groupBranchesByOverlap(thisOriginNewNeurites,
                                                               min_branch_overlap_factor)
                ...
                for group in grouped_branches:
                    ...
                    checkFrequency = True
                    bestBranch = neuriteConstructor.getBestNeuriteBasedOnSimilarityAndFrequency(this_group_df,
                                                                                                self.allNeurites,
                                                                                                checkFrequency)
            ...
            if len(newNeuritesDf) != 0:
                ...
                for newNeuritesRowNb in thisOriginNewNeurites.index.values:
                    ...
                    if thisBranch not in usedBranches:
                        ...
                        thisBranchNewNeurites = newNeuritesDf[(newNeuritesDf.branch == thisBranch) &
                                                              (newNeuritesDf.origin == thisOrigin)]
                        checkFrequency = False
                        bestBranch = neuriteConstructor.getBestNeuriteBasedOnSimilarityAndFrequency(thisBranchNewNeurites,
                                                                                                    allNeurites,
                                                                                                    checkFrequency)
                        ...
                        for newNeuritesBranchRowNb in thisBranchNewNeurites.index.values:
                            nextNeuriteBranch = self.getNextBranchNb(thisOrigin,
                                                                     newNeuritesDf,
                                                                     allNeurites,
                                                                     identity)
    ...
    def checkForOverlapBetweenDifferentOrigins(self,overlappingOrigins,
                                               allNeurites,newNeuritesDf):
        ...
        for overlappingOriginsRowNb in overlappingOrigins.index.values:
            ...
            if overlappingOrigin['newNeuriteNb']:
                ...
                if overlappingOrigin['origin'] != comparedNeurite['origin']:
                    allNeurites.loc[((((allNeurites.date == overlappingOrigin['date']) &
                                       (allNeurites.neuron == overlappingOrigin['neuron'])) &
                                      ((allNeurites.time == overlappingOrigin['time']) &
                                       (allNeurites.channel == overlappingOrigin['channel']))) &
                                     ((allNeurites.origin == overlappingOrigin['origin']) &
                                      (allNeurites.branch == overlappingOrigin['branch']))),'overlap'] = 1
    ...
    def isThereOverlap(self,x1,y1,x2,y2):
        ...
        isOverlap = False
        xOverlap = ((((minX1 >= minX2) & (minX1 <= maxX2)) |
                     ((maxX1 >= minX2) & (maxX1 <= maxX2))) |
                    ((minX1 <= minX2) & (maxX1 >= maxX2)))
        yOverlap = ((((minY1 >= minY2) & (minY1 <= maxY2)) |
    ...
    def getPossibleOriginFromList(self,currentNeuriteCoords,targetNeuritesDf,
                                  possibleOrigins,overlappingOrigins,
                                  currentNeuriteNb,dilationForOverlap,
                                  overlapMultiplicator,frameDifference=0,
                                  threshold=-1,overlapThreshold=-1,
                                  minNbSimilarPx=-1):
#        print("get possible origin from list")
        ...
        startPointOfCurrentNeurite = [currentNeuriteCoords[0][0],
                                      currentNeuriteCoords[1][0]]
        endPointOfCurrentNeurite = [currentNeuriteCoords[0][-1],
                                    currentNeuriteCoords[1][-1]]
        endPointImageOfCurrentNeurite = np.zeros_like(currentNeuriteImage)
        endPointImageOfCurrentNeurite[endPointOfCurrentNeurite[0],
                                      endPointOfCurrentNeurite[1]] = 1
        
        #crop image for speed
        (endPointImageOfCurrentNeurite,
         borderVals1) = generalTools.cropImage(endPointImageOfCurrentNeurite,
                                               [],20)
        endPointImageOfCurrentNeurite = morph.binary_dilation(endPointImageOfCurrentNeurite,
                                                              disk(6))
        #uncrop
        endPointImageOfCurrentNeurite = generalTools.uncropImage(endPointImageOfCurrentNeurite,
                                                                 borderVals1)
        ...
        for dilationForOverlap in range(startDilationForOverlap,maxDilationForOverlap+1):
            if len(targetNeuritesDf) > 0:
                ...
                (currentNeuriteImgCopy,
                 borderVals2) = generalTools.cropImage(currentNeuriteImgCopy,
                                                       [],20)
                currentNeuriteImgCopy = ndimage.binary_dilation(currentNeuriteImgCopy,disk(dilationForOverlap/multiplicator),
                                                                iterations=multiplicator)
...
#                    plt.imshow(thisNeuriteImage)       
                ...
                (thisNeuriteImage,
                 currentNeuriteImgCopy,
                 borderVals3) = generalTools.crop2Images(thisNeuriteImage,
                                                         currentNeuriteImgCopy,20)
                (endPointImageOfCurrentNeurite,
                 borderVals3) = generalTools.cropImage(endPointImageOfCurrentNeurite,
                                                       borderVals3)

                thisNeuriteImage = ndimage.binary_dilation(thisNeuriteImage,
                                                           disk(dilationForOverlap/multiplicator),
                                                           iterations=multiplicator)

                two_way_diff = 1
                (diffOfImgs,
                 overlapOfImgs) = generalTools.overlapImgs(thisNeuriteImage,
                                                           currentNeuriteImgCopy,
                                                           dilationForOverlap,
                ...
                startPointOfThisNeurite = [neurite['x'][0]-borderVals3[0],
                                           neurite['y'][0]-borderVals3[2]]
                lineConnectingStartPoints = line(startPointOfCurrentNeurite[0]-borderVals3[0],
                                                 startPointOfCurrentNeurite[1]-borderVals3[2],
                                                 startPointOfThisNeurite[0],startPointOfThisNeurite[1])
                lineImage = np.zeros_like(thisNeuriteImage)

                lineImage[lineConnectingStartPoints[0],
                          lineConnectingStartPoints[1]] = 1
...
#                plt.imshow(diffOfImgs_noLine)
                ...
                diffOfImgs_labeled, _ = ndimage.label(diffOfImgs,
                                                      structure=[[1,1,1],
                                                                 [1,1,1],
                                                                 [1,1,1]])

                endPointOfThisNeurite = [neurite['x'][-1]-borderVals3[0],
                                         neurite['y'][-1]-borderVals3[2]]
                endPointImageOfThisNeurite = np.zeros_like(thisNeuriteImage)
                endPointImageOfThisNeurite[endPointOfThisNeurite[0],
                                           endPointOfThisNeurite[1]] = 1

                #crop for speed and after one operation uncrop
                (endPointImageOfThisNeurite,
                 borderVals4) = generalTools.cropImage(endPointImageOfThisNeurite,
                                                       [],20)
                endPointImageOfThisNeurite = morph.binary_dilation(endPointImageOfThisNeurite,
                                                                   disk(6))
                endPointImageOfThisNeurite = generalTools.uncropImage(endPointImageOfThisNeurite,
                                                                      borderVals4)
                ...
                gain_of_imgs[(diffOfImgs_labeled != endPointLabelCurrentNeurite) &
                             (diffOfImgs_labeled != endPointLabelThisNeurite)] = 0
                loss_of_imgs[(diffOfImgs_labeled != endPointLabelCurrentNeurite) &
                             (diffOfImgs_labeled != endPointLabelThisNeurite)] = 0
                ...
                (labeledDiffOfImgs,
                 nbOfIslands) = ndimage.label(diffOfImgs,
                                              structure=[[1,1,1],
                                                         [1,1,1],
                                                         [1,1,1]])
...
#                    plt.text(40,40,pxOverlap)
                ...
                currentNeuriteImgCopy = generalTools.uncropImage(currentNeuriteImgCopy,
                                                                 borderVals3)
                endPointImageOfCurrentNeurite = generalTools.uncropImage(endPointImageOfCurrentNeurite,
                                                                         borderVals3)

                #uncrop images for saving coord data
                gain_of_imgs = generalTools.uncropImage(gain_of_imgs,
                                                        borderVals3)
                loss_of_imgs = generalTools.uncropImage(loss_of_imgs,
                                                        borderVals3)


                if (pxOverlap > overlapThreshold) & (frameDifference < 4):
                    overlappingOriginsNb = len(overlappingOrigins)
                    overlappingOrigins.loc[overlappingOriginsNb] = [currentNeuriteNb,neurite['date'],
                                                                    neurite['experiment'],
                                                                    neurite['neuron'],
                                                                    neurite['channel'],
                                                                    neurite['time'],
                ...
                if (pxOverlap > (minNbSimilarPx*0.8)):
                    ...
                    if len(np.unique(gain_of_imgs)) > 1:
                        sorted_gain_coords = self.get_sorted_coords_from_img(gain_of_imgs,
                                                                             ref_coords,
                                                                             self.minBranchSize)
                    else:
                        sorted_gain_coords = [[],[]]
                    if len(np.unique(loss_of_imgs)) > 1:
                        sorted_loss_coords = self.get_sorted_coords_from_img(loss_of_imgs,
                                                                             ref_coords,
                                                                             self.minBranchSize)
                    else:
                        sorted_loss_coords = [[],[]]
                    ...
                    possibleOrigins.loc[possibleOriginsNb] = [neurite['origin'],
                                                              neurite['branch'],
                                                              neurite['start_branch'],
                                                              neurite['x'],
                                                              neurite['y'],
                                                              sorted_gain_coords[0],
                                                              sorted_gain_coords[1],
                                                              sorted_loss_coords[0],
                                                              sorted_loss_coords[1],
                                                              pxDifference,
                                                              overlapRatio,
                                                              pxOverlap,
                                                              neurite['avIntOfPoints']]
                    oneOriginFound = True
            if oneOriginFound:
                break
    ...
    def remove_gained_and_add_lost_px(self,this_neurite_image,neurite):
        #add some of the px that were gained or lost in image to compare to 
        #gained or lost relative to the image which
        # was the best compared image for the image currently compared to
        nb_px_to_remove = int(np.round(min(len(neurite['x'])*0.1,
                                           self.minBranchSize*0.5),0))
