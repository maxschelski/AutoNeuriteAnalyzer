...
class SeparateNeurites:
    ...
    @staticmethod
    def separateNeuritesByOpening(timeframe_neurites, maxNeuriteOpening,
                                  timeframe_soma, img, grainSizeToRemove,
                                  contrastThresholdToRemoveLabel,
                                  maxFractionToLoseByOpening):

        (timeframe_neurites,
         borderVals) = generalTools.cropImage(timeframe_neurites,[],10)
        (timeframe_soma,
         borderVals) = generalTools.cropImage(timeframe_soma,borderVals)
        img, borderVals = generalTools.cropImage(img,borderVals)
        
        timeframe_neurites[timeframe_soma == True] = False;
        timeframe_labeled,_ = ndimage.label(timeframe_neurites,
...
#                    print("erosion: {}".format(a))
                oneNeurite_test = ndimage.morphology.binary_opening(oneNeurite,
                                                                    square(openingSize))
                oneNeurite_test[timeframe_soma == True] = 0
                (oneNeurite_labeled,
                 nbOfLabels_oneNeurite) = ndimage.label(oneNeurite_test,
                                                         structure=[[1,1,1],
                                                                    [1,1,1],
                                                                    [1,1,1]])
                ...
                if nbOfLabels_oneNeurite > 1:
                    ...
                    if nbOfLabels_oneNeurite == 1:
                        ...
                        (pxToKeep,pxToRemove,
                         pxToRemove_contrast,
                         removedOneLabel,
                         forceApplyOpening) = SeparateNeurites.checkWhichPxDontSeparateNeurites(oneNeurite_test,
                                                                                                oneNeurite,
                                                                                                timeframe_soma,
                                                                                                timeframe_labeled,
                                                                                                label,img)
            ...
            if applyOpening:
                oneNeurite[timeframe_soma == True] = 1
                oneNeurite = ndimage.morphology.binary_opening(oneNeurite,
                                                               square(openingSizeToApply))

                timeframe_neurites[(oneNeurite == 0) &
                                   (timeframe_labeled == label)] = 0
            if removedOneLabel:
                ...
                if maxContrast/100 > contrastThresholdToRemoveLabel:
                    ...
                    oneNeurite = SeparateNeurites.separateNeuritesEvenly(oneNeurite,
                                                                         pxToRemove)
                timeframe_neurites[(oneNeurite == 0) &
                                   (timeframe_labeled == label)] = 0
    ...
    @staticmethod
    def checkWhichPxDontSeparateNeurites(oneNeurite_test,oneNeurite,
                                         timeframe_soma,timeframe_labeled,label,img):
        

        #-------------CROP IMAGES FOR SPEED-------------
        oneNeurite,borderVals = generalTools.cropImage(oneNeurite,[],30)
        oneNeurite_test,borderVals = generalTools.cropImage(oneNeurite_test,
                                                            borderVals)
        timeframe_soma,borderVals = generalTools.cropImage(timeframe_soma,
                                                           borderVals)
        timeframe_labeled,borderVals = generalTools.cropImage(timeframe_labeled,
                                                              borderVals)
        img,borderVals = generalTools.cropImage(img,borderVals)
        

        #if more labels were present, check each part of px
        # that were removed and their intensity
        #measure intensity as fold change from neighbouring px
        # (take all sites at which it touches px separately)
        ...
        oneNeurite_removedPx_labeled, _ = ndimage.label(oneNeurite_removedPx,
                                                        structure=[[1,1,1],
                                                                   [1,1,1],
                                                                   [1,1,1]])
        ...
        for label_removed in np.unique(oneNeurite_removedPx_labeled):
            if label_removed <= 0:
                continue
            #test if removing the current label from original image of neurite,
            # increases nb of labels (seperates neurites)
            ...
            (oneNeurite_removedLabel_labeled,
             nbLabelsRemovedLabel) = ndimage.label(oneNeurite_removedLabel,
                                                   structure=[[1,1,1],
                                                              [1,1,1],
                                                              [1,1,1]])
            ...
            #since things that were remoced actually led to splitting and
            # therefore loss of some part/s of neurites
            forceApplyOpening = False

            #get contast of island to neighhboring pixels
            #contrast helps to remove only parts of mini-neurites
            # between bigger neurites without separating bigger neurites in the middle
            averagePxVal = np.median(img[oneLabel == 1])
            oneLabel_dilated = ndimage.morphology.binary_dilation(oneLabel,
                                                                  disk(10))
            oneLabel_neighbors = np.zeros_like(oneNeurite)
            oneLabel_neighbors[((oneLabel_dilated == 1) &
                                (oneNeurite == 1)) &
                               (oneLabel == 0)] = 1
            oneLabel_neighbors = ndimage.morphology.binary_erosion(oneLabel_neighbors,
                                                                   disk(2))
            oneLabel_neighbors_labeled, _ = ndimage.label(oneLabel_neighbors,
                                                          structure=[[1,1,1],
                                                                     [1,1,1],
                                                                     [1,1,1]])
            maxContrast = 0
            for neighbor_label in np.unique(oneLabel_neighbors_labeled):
                if neighbor_label != 0:
                    averagePxValOfNeighbor = np.median(img[oneLabel_neighbors_labeled ==
                                                           neighbor_label])
            ...
            (oneNeurite_removedLabel_soma_labeled,
             nbLabelsWithSoma) = ndimage.label(oneNeurite_removedLabel_soma,
                                              structure=[[1,1,1],
                                                         [1,1,1],
                                                         [1,1,1]])
            ...
            if (nbLabelsWithSoma > 1) | (maxSizeOfLabels > 0.9 * sizeOneNeurite):
                ...
                pxToRemoveContrast[oneLabel == 1] = int(np.round(maxContrast*100,
                                                                 0))
        ...
        pxToRemoveContrast = generalTools.uncropImage(pxToRemoveContrast,
                                                      borderVals)
            
        return (pxToKeep,pxToRemove, pxToRemoveContrast,
                removedOneLabel, forceApplyOpening)

    @staticmethod
    def separateNeuritesEvenly(oneNeurite,pxToRemove):
        pxToRemove_labeled,_ = ndimage.label(pxToRemove,
                                            structure=[[1,1,1],
                                                       [1,1,1],
                                                       [1,1,1]])
        allSizeRatios = []
        ...
        for labelToRemove in np.unique(pxToRemove_labeled):
            ...
            oneNeurite_test_labeled,_ = ndimage.label(oneNeurite_test,
                                                     structure=[[1,1,1],
                                                                [1,1,1],
                                                                [1,1,1]])
            allNeuriteSizes = []
            for oneNeuriteLabel in np.unique(oneNeurite_test_labeled):
                if oneNeuriteLabel != 0:
                    neuriteSize = len(np.where(oneNeurite_test_labeled ==
                                               oneNeuriteLabel)[0])
        ...
        finalLabelToRemove = allLabelToRemove[np.where(allSizeRatios ==
                                                       np.min(allSizeRatios))[0][0]]
    ...
    @staticmethod
    def separateNeuritesBySomaDilation(timeframe_neurites,
                                       timeframe_soma,
                                       maxSomaDilation):
        ...
        for a in range(1,maxSomaDilation):
            timeframe_labeled,_ = ndimage.label(timeframe_neurites,
                                                 structure=[[1,1,1],
                                                            [1,1,1],
                                                            [1,1,1]])
            timeframe_soma_dilated = ndimage.morphology.binary_dilation(timeframe_soma,
                                                                        disk(a))
            for label in np.unique(timeframe_labeled):
                if (label == 0):
                    continue
                ...
                oneLabel_labeled, nbOfLabelsBefore = ndimage.label(oneLabel,
                                                                   structure=[[1,1,1],
                                                                              [1,1,1],
                                                                              [1,1,1]])
                oneLabel_noSoma = copy.copy(oneLabel)
                oneLabel_noSoma[timeframe_soma_dilated == 1] = 0
                oneLabel_labeled, nbOfLabelsAfter = ndimage.label(oneLabel_noSoma,
                                                                  structure=[[1,1,1],
                                                                             [1,1,1],
                                                                             [1,1,1]])
                ...
                oneLabel_opened = ndimage.morphology.binary_opening(oneLabel,
                                                                    disk(a/2+2))

                timeframe_finalSoma[(timeframe_soma == 0) &
                                    (timeframe_soma_dilated == 1) &
                                    (oneLabel_opened == 0) &
                                    (oneLabel == 1)] = 1
                timeframe_neurites[(timeframe_soma == 0) &
                                   (timeframe_soma_dilated == 1) &
                                   (oneLabel_opened == 0) &
                                   (oneLabel == 1)] = 0
